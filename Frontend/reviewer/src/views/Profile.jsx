...
export default function AccountPage(props) {
    ...
    async function updateDescription() {
        ...
        const token = JSON.parse(JSON.stringify(decoded))
          try {
            await fetch(URL + "update-description/" + token.sub, {
              method: "POST",
    ...
    async function updateProfile() {
        ...
        const token = JSON.parse(JSON.stringify(decoded))
          try {
            await fetch(URL + "upload-image/" + token.sub, {
              method: "POST",
              mode: "cors", // no-cors, *cors, same-origin 
              credentials: "same-origin", // include, *same-origin, omit
              headers: { "Authorization": `Bearer ${jwt_token}` },
    ...
    React.useEffect(() => {
      ...
      async function fetchData(url = "") {
        ...
        const token = JSON.parse(JSON.stringify(decoded))
          try {
            await fetch(url + "user/" + token.sub, {
              method: "GET",
              mode: "cors", // no-cors, *cors, same-origin 
              credentials: "same-origin", // include, *same-origin, omit
              headers: { "Authorization": `Bearer ${jwt_token}` },
              ...
              .then(text =>  {
                  setUser(JSON.parse(text).user.username)
                  setReviews(JSON.parse(text).reviews)
                  setUserDescription(JSON.parse(text).user.description)
            }
                )
          }
        ...
        <div className="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4">
            <div className="col-span-4 sm:col-span-3">
                <div className="bg-white shadow rounded-lg p-6">
                    <div className="flex flex-col items-center">
                        <img 
                            src={"http://localhost:8080/api/download-image/" + user}
                        ...
                        <div className="mt-6 flex flex-wrap gap-4 justify-center">
                            <a onClick={() => setShowModal(true)}
                             className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded cursor-pointer">Edit Profile</a>
                        </div>
                    </div>
                    <div className="modal-example">
                        ...
                        >
                            <div>
                                ...
                                <div className="modal-desc justify-center">
                                    <div className='flex items-center justify-center'>
                                        <div className='modal-profile-picture-overlay'>
                                                <div className=' h-0 overflow-hidden' >
                                                    <input id="fileInput" type='file' formEncType='multipart/form-data' onChange={handleChange}/>
                                                </div>
                                                <button onClick={changePicture} className='relative left-6 top-6 cursor-pointer justify-center items-center w-8 h-8'><img src={CameraIcon}></img></button>
                                        </div>
                                        <img
                                            src={"http://localhost:8080/api/download-image/" + user } 
                                        ...
                                        <form className=' grid-flow-row justify-center'>
                                            <textarea id='description' onChange={changeDescription} maxLength={160} className=' min-w-full overflow-hidden min-h-full resize-none p-10' placeholder='Enter your description here'/>
                                        </form>
                                </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="inline-block">
                            <div className="w-20 h-8 overflow-hidden">
                                <button className="text-gray-600 hover:font-bold tracking-wider mb-2">edit</button>
                            </div>
                        </div>
            ...
            <div className="col-span-4 sm:col-span-9">
                <div className="bg-white shadow rounded-lg p-6">
                    ...
                    <p className="text-gray-700">{userDescription}
                    </p>
                    ...
                    <div className="mb-6">
                        {reviews.map(review => <h1>{review.message}</h1>)}
                    </div>
                </div>
            </div>
        </div>
